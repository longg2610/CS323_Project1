        ...
        for height in heights:
            for weight in weights:
                ...
                if count < k and count > 0:
                    print("k-anonymity unsatisfied: QID (", age, height, weight, ") appeared", count, "times.", end = " ")
                    return (False, -1)
    return (True, penalty)
...
def main():
    ...
    # perform search for the best generalization node, updating penalty_dict in the process
    anonymize(df, k, [0,0,0])      # 2, 3, 20
    ...
    print("The generalization node", min_key, "yields the lowest penalty of", penalty_dict[min_key])

    # anonymize the outputted data using the lowest penalty node
    ...
    # final check before writing to csv
    ...
    if len(violating_groups) > 0:
        print(f"Data does NOT satisfy {k}-anonymity.")
        print("Violating QID combinations and their counts:")
        print(violating_groups)
    else:
        print(f"Data satisfies {k}-anonymity.")
